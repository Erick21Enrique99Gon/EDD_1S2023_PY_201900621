...
class BlockChain{
    ...
    async insert(transmitter, receiver, message){
        ...
        if(this.head == null){
            ...
            newNode.mensaje = await this.getSha256(message);
            // INSERTAR EL NODO
            this.head = newNode;
            this.end = newNode;
            // AUMENTAR TAMAÑO
            this.size++;
            return { end: this.end, size: this.size };
        }else{
            ...
            while(temp !== null){
                if(String(temp.receiver) === String(transmitter)){
                    ...
                    if(String(temp.receiver) === String(receiver)){
                        msgs += `<li class="list-group-item bg-success text-light" style="text-align: right">${temp.message}</li>`;
                    }
                }
                temp = temp.next;
            }
