...
function loadHash() {
    if (localStorage.getItem("hashTable") !== null) {
        let temp = JSON.retrocycle(JSON.parse(localStorage.getItem("hashTable")));
        hashtable.table = temp.table;
        hashtable.capacidad = temp.capacidad;
        hashtable.espaciosUsados = temp.espaciosUsados;
    }
    if (localStorage.getItem("avlTree") !== null) {
        let temp = JSON.retrocycle(JSON.parse(localStorage.getItem("avlTree")));
        avlTree.root = temp.root;
        hashtable = avlTree.insetarHash(hashtable);
        localStorage.setItem("hashtable", JSON.stringify(JSON.decycle(hashtable)));
        console.log(hashtable);
        let body = hashtable.getHTML();
        $("#studentsHashTable tbody").html(
            body
        )
    } else {
...
function updateChats() {
    if (localStorage.getItem("blockChain") !== null) {
        ...
        blockChain.head = temp.head;
        blockChain.end = temp.end;
        blockChain.size = temp.size;
    }
    estudiante = JSON.parse(localStorage.getItem("estudiante"));
    let receiver = $('#traversalCarnets').val();
    $('#transmitter-chat').html(blockChain.getMessages(estudiante.carnet, receiver));
}
...
async function sendMessage() {
    ...
    let transmitter = JSON.parse(localStorage.getItem("estudiante")).carnet;
    let receiver = $('#traversalCarnets').val();
    ...
    if (transmitter && receiver) {
        ...
        alert("No ha seleccionado Receptor");
    }
}

function showGraphChainBlock() {
    if (localStorage.getItem("blockChain") !== null) {
        let temp = JSON.retrocycle(JSON.parse(localStorage.getItem("blockChain")));
        blockChain.head = temp.head;
        blockChain.end = temp.end;
